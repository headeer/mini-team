...
function convertToCsv(products: BaselinkerProductWithParams[]): string {
  ...
  for (const product of products) {
    const row = [
      ...
      product.param_WeightRange,
      product.param_Subcategory,
      product.param_PhoneOrderOnly,
      product.param_TeethEnabled,
      product.param_ToothCost,
      product.param_ToothQty,
      product.param_MountSystems,
      product.param_Dimension_A,
      product.param_Dimension_B,
      product.param_Dimension_C,
      product.param_Dimension_D,
      product.param_Brand,
      product.param_Status,
      product.param_ExternalId,
      product.param_WidthCm,
      product.param_PinDiameterMm,
      product.param_VolumeM3,
      product.param_CuttingEdge,
      product.param_ToothThickness,
      product.param_ToothCount,
      product.param_Features,
      product.param_MachineCompatibility,
      product.param_QuickCoupler,
      product.param_Kinetyka,
      product.param_Ramie,
      product.param_TechnicalDrawingUrl,
      product.param_TechnicalDrawingTitle,
      product.param_TechnicalDrawingsCodes,
      product.param_PriceNet,
      product.param_PriceOlx,
      product.param_PriceText,
      product.param_Discount,
...
async function main() {
  try {
    ...
    console.log('🔄 Generating CSV without headers...')
    ...
    // Write CSV file without headers
    const timestamp = new Date().toISOString().replace(/[:.]/g, '-').slice(0, 19)
    const filename = `baselinker-no-headers-${timestamp}.csv`
    const filepath = path.join(exportsDir, filename)
    
    fs.writeFileSync(filepath, csvContent, 'utf8')
    
    console.log(`✅ Baselinker CSV without headers exported: ${filepath}`)
    console.log(`📊 Exported ${baselinkerProducts.length} products`)
    console.log(`📋 Total columns: 49 (no headers)`)
    
    // Also update the main file
    const mainFilepath = path.join(exportsDir, 'baselinker-no-headers.csv')
