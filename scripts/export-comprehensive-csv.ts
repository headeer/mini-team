...
async function main() {
  try {
    ...
    console.log('🔄 Converting to comprehensive format...')
    ...
    console.log(`✅ Comprehensive CSV exported successfully: ${filepath}`)
    console.log(`📊 Exported ${comprehensiveProducts.length} products with all parameters`)
    console.log(`📋 Total columns: ${Object.keys(comprehensiveProducts[0] || {}).length}`)
    
    // Also create a Baselinker-specific version
    const baselinkerFilename = `baselinker-comprehensive-${timestamp}.csv`
    const baselinkerFilepath = path.join(exportsDir, baselinkerFilename)
    fs.writeFileSync(baselinkerFilepath, csvContent, 'utf8')
    console.log(`📄 Baselinker version: ${baselinkerFilepath}`)
